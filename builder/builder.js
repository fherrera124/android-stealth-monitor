...
app.post('/build', async (req, res) => {
  ...
  if (!configUrl) {
    return res.status(400).json({ success: false, error: 'Config URL required' });
  }
  console.log('Build request received with Config URL:', configUrl);

  const command = `./gradlew assembleDebug -PconfigUrl='${configUrl}' --no-daemon --stacktrace`;
